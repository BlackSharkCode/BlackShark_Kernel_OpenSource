...
#define LMK_DEATHPENDING_TICKS (HZ / 4)
...
static unsigned long lowmem_scan(struct shrinker *s, struct shrink_control *sc)
{
		...
		if (oom_reaper) {
			...
			if (time_before_eq(jiffies,
				...
				if (test_task_lmk_waiting(tsk)) {
				    lowmem_print(4, "still waiting for task '%s' (%d), return\n",
					    tsk->comm, tsk->pid);
					rcu_read_unlock();
					mutex_unlock(&scan_mutex);
					return 0;
	...
	for (i = 0; i < LMK_DEPTH; i++) {
		if (selected[i]) {
			...
			if (test_task_lmk_waiting(selected[i]) &&
			...
			trace_lowmemory_kill(selected[i], cache_size,
					     cache_limit, free);
			lowmem_print(1, "Multiple LMK: #%d\n"
				"Killing %s (%d) (tgid %d), adj %d,\n"
			...
			lowmem_deathpending_timeout = jiffies +
				LMK_DEATHPENDING_TICKS;
