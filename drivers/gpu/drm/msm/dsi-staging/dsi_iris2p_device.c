/* zeusis Fast Capture Driver
 *
 * Copyright (C) 2016 Zeusis Device Co.Ltd
...
/*
 * factory pt test
 * enable dsp power: adb echo pt 1 > /dev/iris2p
 * disable dsp power: adb echo pt 0 > /dev/iris2p
 *
 */
			    ...
			    char *buf)
{
	...
	if (iris_info.lut_info.lut_fw_state && iris_info.gamma_info.gamma_fw_state) {
		if (iris_p->mcf_value == DEFAULT_MCF) {
			strcpy(name, "DEFAULT_MCF");
		} else if (iris_p->mcf_value == ACTIVE_MCF) {
			strcpy(name, "ACTIVE_MCF");
		}
	} else {
		strcpy(name, "NO_MCF");
	}
...
* bit[0]: 1:miss; te 0:normal
* bit[1]: 1:has been lose te before; 0:normal
* bit[2~3]:no use
* bit[4~7]:iris internel detect count
