...
static int handle_jeita(struct step_chg_info *chip)
{
	...
	rc = get_val(chip->aging_running ? chip->jeita_fcc_config->fcc_cfg_aging : chip->jeita_fcc_config->fcc_cfg,
			chip->jeita_fcc_config->hysteresis,
			chip->jeita_fcc_index,
			pval.intval,
			&chip->jeita_fcc_index,
			&fcc_ua,
			chip->aging_running ? chip->jeita_fcc_config->aging_array_len: chip->jeita_fcc_config->array_len);
	if (rc < 0) {
		/* remove the vote if no step-based fcc is found */
		if (chip->fcc_votable)
			vote(chip->fcc_votable, JEITA_VOTER, false, 0);
	...
	rc = get_val(chip->aging_running ? chip->jeita_fv_config->fv_cfg_aging : chip->jeita_fv_config->fv_cfg,
			chip->jeita_fv_config->hysteresis,
			chip->jeita_fv_index,
			pval.intval,
			&chip->jeita_fv_index,
			&fv_uv,
			chip->aging_running ? chip->jeita_fv_config->aging_array_len: chip->jeita_fv_config->array_len);
	if (rc < 0) {
		/* remove the vote if no step-based fcc is found */
		if (chip->fv_votable)
			vote(chip->fv_votable, JEITA_VOTER, false, 0);
		...
		bool step_chg_enable, bool sw_jeita_enable)
{
	...
	pr_info("qcom_step_chg_init successfully chip->step_chg_enable=%d chip->sw_jeita_enable=%d\n",
