...
static u8 firmware_data[] = {
	#include "touch_firmware.h"
};

struct nvt_firmware_data {
	size_t size;
	u8 *data;
};

static struct nvt_firmware_data *fw_entry = NULL;
...
void Boot_Update_Firmware(void)
{
	...
	if (!fw_entry) {
		NVT_ERR("\"NVT\" fw zalloc failed !\n");
		return;
	}

	fw_entry->data = &firmware_data[0];
	fw_entry->size = sizeof(firmware_data);
	...
	if (sizeof(firmware_data) != FW_BIN_SIZE) {
		NVT_ERR("firmware_data size not match. (%zu)\n", sizeof(firmware_data));
		return;
	}

	// check if FW version add FW version bar equals 0xFF
	if (*(&firmware_data[0] + FW_BIN_VER_OFFSET) + *(&firmware_data[0] + FW_BIN_VER_BAR_OFFSET) != 0xFF) {
		NVT_ERR("bin file FW_VER + FW_VER_BAR should be 0xFF!\n");
		NVT_ERR("FW_VER=0x%02X, FW_VER_BAR=0x%02X\n", *(&firmware_data[0] + FW_BIN_VER_OFFSET), *(&firmware_data[0] + FW_BIN_VER_BAR_OFFSET));
		return;
	}
...
Description:
	...
	sector_num: 0: sector with start address 0x0000, 
	1: sector with start address 0x1000
return:
	Executive outcomes. 0---succeed. negative---failed.
*******************************************************/
int32_t Sector_Erase(int32_t sector_num)
{
	uint8_t buf[64] = {0};
	int32_t ret = 0;
	int32_t Flash_Address = 0;
	...
	if ((sector_num < 0) || (sector_num > 63)) {
		NVT_ERR("Invalid sector number: %d\n", sector_num);
		return -EINVAL;
	}
	Flash_Address = sector_num << 12;
	NVT_LOG("Erase Sector %d, Flash_Address = 0x%X\n", sector_num, Flash_Address);
	...
	while (1) {
		...
		ret = CTP_I2C_READ(ts->client, I2C_HW_Address, buf, 2);
		if (ret < 0) {
			NVT_ERR("Check 0xAA (Write Enable) error!!(%d)\n", ret);
			return ret;
		}
		if (buf[1] == 0xAA) {
			break;
	...
	while (1) {
		...
		ret = CTP_I2C_READ(ts->client, I2C_HW_Address, buf, 2);
		if (ret < 0) {
			NVT_ERR("Check 0xAA (Sector Erase) error!!(%d)\n", ret);
			return ret;
		}
		if (buf[1] == 0xAA) {
			break;
...
return:
	Executive outcomes. 0---succeed. negative---failed.
*******************************************************/
int32_t Fast_Read_Checksum(uint32_t flash_address, int32_t length, uint16_t *checksum)
{
	uint8_t buf[64] = {0};
	uint32_t XDATA_Addr = ts->mmap->READ_FLASH_CHECKSUM_ADDR;
	int32_t ret = 0;
	...
	buf[3] = ((flash_address >> 8) & 0xFF);
	buf[4] = (flash_address & 0xFF);
	buf[5] = (((length - 1) >> 8) & 0xFF);
	buf[6] = ((length - 1) & 0xFF);
	ret = CTP_I2C_WRITE(ts->client, I2C_HW_Address, buf, 7);
	if (ret < 0) {
		NVT_ERR("Fast Read Checksum error!!(%d)\n", ret);
		return ret;
	}
	// Check 0xAA (Fast Read Checksum)
	retry = 0;
	while (1) {
		mdelay(80);
		buf[0] = 0x00;
		...
		ret = CTP_I2C_READ(ts->client, I2C_HW_Address, buf, 2);
		if (ret < 0) {
			NVT_ERR("Check 0xAA (Fast Read Checksum) error!!(%d)\n", ret);
			return ret;
		}
		if (buf[1] == 0xAA) {
			break;
		...
		retry++;
		if (unlikely(retry > 5)) {
			NVT_ERR("Check 0xAA (Fast Read Checksum) failed, buf[1]=0x%02X, retry=%d\n", buf[1], retry);
			return -1;
		}
	}
	...
	// Read Checksum
	buf[0] = 0xFF;
	buf[1] = (XDATA_Addr >> 16) & 0xFF;
	buf[2] = (XDATA_Addr >> 8) & 0xFF;
	ret = CTP_I2C_WRITE(ts->client, I2C_BLDR_Address, buf, 3);
	if (ret < 0) {
		NVT_ERR("Fast Read Checksum change I2C buffer index error!!(%d)\n", ret);
		return ret;
	}
	...
	*checksum = (uint16_t)((buf[2] << 8) | buf[1]);

	NVT_LOG("Fast Read Checksum OK, checksum = 0x%04X\n", *checksum);
	return 0;
}
