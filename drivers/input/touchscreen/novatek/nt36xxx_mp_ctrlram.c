...
static int32_t c_show_selftest(struct seq_file *m, void *v)
{
	...
	if((TestResult_Short == 0) && (TestResult_Open == 0) && \
		...
		nvt_mp_seq_printf(m, "P");
	}
	else{
		nvt_mp_seq_printf(m, "F");
		nvt_mp_seq_printf(m, "\n");
...
static int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
    ...
    //touchscreen suspend, can't selftest
    if(!bTouchIsAwake)
    {
        NVT_ERR("Touchscreen has suspend,TouchIsAwake:%d.\n", bTouchIsAwake);
        return -ERESTARTSYS;
    }
...
int32_t nvt_mp_proc_init(void)
{
	...
	{
		NVT_ERR("Make /proc/touchscreen dir failed\n");
		return -2;
	}

	NVT_proc_selftest_entry = proc_create("touchscreen/rawdata", S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH, NULL, &nvt_selftest_fops);
	if (NVT_proc_selftest_entry == NULL) {
		NVT_ERR("create /proc/touchscreen/rawdata Failed!\n");
		return -1;
	} else {
...
#endif
			NVT_LOG("create /proc/touchscreen/rawdata Succeeded!\n");
		}
		return 0;
	}
}
