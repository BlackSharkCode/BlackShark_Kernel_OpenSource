...
#define AW2015_LED_ALL_MASK				0x07
#define INITCMD		 					0x00
...
static ssize_t aw2015_store_mcu_control(struct device *dev,
			     struct device_attribute *attr,
			     const char *buf, size_t len)
{
	int brightness = 0;
	struct aw2015_led *led = dev_get_drvdata(dev);
	u8 val = 0;

	sscanf(buf, "%x", &brightness);

	if(brightness == INITCMD){
		/* enable aw2015 if disabled */
		aw2015_read(led, AW2015_REG_GCR, &val);
		if (!(val&AW2015_LED_CHIP_ENABLE_MASK)) {
			aw2015_write(led, AW2015_REG_GCR, AW2015_LED_CHIP_ENABLE_MASK | AW2015_LED_CHARGE_DISABLE_MASK);
		...
		val = (u8)brightness;
		aw2015_write(led, AW2015_REG_LCFG1, AW2015_LED_ON_MODE_MASK);
		aw2015_write(led, AW2015_REG_LCFG2, AW2015_LED_ON_MODE_MASK);
		aw2015_write(led, AW2015_REG_LCFG3, AW2015_LED_ON_MODE_MASK);
		aw2015_write(led, AW2015_REG_IMAX , led->pdata->imax);
		aw2015_write(led, AW2015_REG_ILED1, 0);
		aw2015_write(led, AW2015_REG_ILED2, 0);
		aw2015_write(led, AW2015_REG_ILED3, 0);
		aw2015_write(led, AW2015_REG_PWM1, AW2015_LED_ON_PWM_MASK);
		aw2015_write(led, AW2015_REG_PWM2, AW2015_LED_ON_PWM_MASK);
		aw2015_write(led, AW2015_REG_PWM3, AW2015_LED_ON_PWM_MASK);
		aw2015_write(led, AW2015_REG_LEDEN, AW2015_LED_ALL_MASK);

		return len;
	}

	val = brightness&0xff;
	switch((brightness&0xffff)>>8){
	case 0:
		aw2015_write(led, AW2015_REG_ILED1, val);
		break;
	case 1:
		aw2015_write(led, AW2015_REG_ILED2, val);
		break;
	case 2:
		aw2015_write(led, AW2015_REG_ILED3, val);
		break;
	case 3:
		aw2015_write(led, AW2015_REG_ILED1, val);
		aw2015_write(led, AW2015_REG_ILED2, val);
		...
		break;
	default:
		pr_info("AW92015 shark mcu_control not set current.\n");
		break;
	}
...
static DEVICE_ATTR(mcu_control, 0664, NULL, aw2015_store_mcu_control);
			   ...
			   const struct i2c_device_id *id)
{
	...
	ret = device_create_file(&led_array->client->dev, &dev_attr_mcu_control);
    if(ret < 0)
    {
        pr_err("create mcu_control attr error\n");
    }

	ret = sysfs_create_link(NULL, &led_array->client->dev.kobj, "rgb");
	if (ret){
		pr_err("Error %d creating rgb link\n",ret);
	}

    exlight_create_sys_node(led_array);
    exlight_boot_up_light_on(led_array);
...
#include "leds-exlight.injection.c"
