			...
			struct kobj_attribute *attr, char *buf)
{
	...
	if (v == -1)
		return snprintf(buf, 18, "unknown poweroff\n");
...
static int __init bootinfo_init(void)
{
	...
	if (bootinfo_kobj == NULL) {
		pr_err("[%s]:kobject create and add bootinfo failed\n", __func__);
		goto fail;
	}
	...
	if (ret) {
		pr_err("[%s]:sysfs create bootinfo group failed\n", __func__);
		goto sys_fail;
	}

	pr_err("[%s]:bootinfo init success\n", __func__);
	return ret;
